...
const Navbar = () => {
    ...
    const toggleIsOpen=()=>setIsMobileOpen(!isMobileOpen)
  return (
    <>
    <div className="bg-green-600 text-white py-3 px-4 sm:px-6 lg:px-8 flex items-center justify-between shadow-lg sticky top-0 z-50 font-sans gap-3">
        <div className='sm:hidden'>
					<Menu className='size-6 cursor-pointer'/>
				</div>
      {isMobileOpen && <div className='w-full sm:hidden mt-4 z-50 bg-black border rounded border-gray-800'>
					<a to={"/"} className='block hover:underline p-2'>
						Movies
					</a>
					<a to={"/"} className='block hover:underline p-2'>
						Tv Shows
					</a>
					<a to={"/history"} className='block hover:underline p-2'>
						Search History
					</a>
				</div>}
      {/* Logo */}
    ...
    <div className='flex items-center justify-center gap-10 flex-shrink-0'>
      ...
      {/* Login Button */}
      <button className=" cursor-pointer bg-transparent border border-white text-white text-sm py-1.5 px-3 sm:py-2 sm:px-4 rounded-full hover:bg-green-700 hover:border-green-700 transition-colors duration-200">
        Login
      </button>
    </div>
    </div>
